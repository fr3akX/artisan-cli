...
func TestInventoryAdjustConfirmationIncludesExactChangeAndSendsCanonicalBody(t *testing.T) {
	...
	result := runAuthCommand(t, runtime, "inventory", "adjust", commandLotID, "--grams", "-125", "--reason", "physical count", "--reference", "sheet", "--occurred-at", "2026-08-07T12:34:56.000000Z", "--idempotency-key", "advanced-key")
	if result.code != 0 {
	...
	if body != `{"quantity_grams":-125,"reason":"physical count","reference":"sheet","occurred_at":"2026-08-07T12:34:56.000000Z"}` || key != "advanced-key" {
